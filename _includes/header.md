<header class="site-header" data-site-header>
  <div class="wrap">
    <nav class="site-nav" aria-label="주요 메뉴">
      ...
      <a class="brand" href="{{ '/' | relative_url }}" aria-label="PageRivet 홈"><span class="mark" aria-hidden="true"></span><span>PageRivet</span></a>
      <div class="mobile-menu-panel" id="site-navigation" data-mobile-menu-panel>
        <div class="links">
          {% for item in site.data.navigation.items %}
            <a href="{{ item.url | relative_url }}"{% if page.nav == item.id %} aria-current="page"{% endif %} data-i18n="navigation.{{ item.id }}">{{ item.label_ko }}</a>
          {% endfor %}
        ...
        <div class="tools">
          ...
          <button class="chip on is-active" type="button" data-language="ko" aria-pressed="true">한국어</button>
